...
function MessageRow({ msg, agentName }: { msg: ChatMessage; agentName?: string | null }) {
...
export function ChatPage(_props: PluginPageProps) {
  ...
  const [selectedAdapter, setSelectedAdapter] = useState("openclaw_gateway");
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="chat-scroll flex-1 overflow-auto px-8 relative">
          ...
          {selectedThreadId && messages?.map((msg) => (
            <MessageRow key={msg.id} msg={msg} agentName={selectedThread?.agentName ?? null} />
          ))}
          ...
          {selectedThreadId && isStreaming && (
            <StreamingMessage
              ...
              isActive={true}
              agentName={selectedThread?.agentName ?? null}
            />
          )}
