...
async function authPost(path: string, body: Record<string, unknown>) {
  const res = await fetch(`${((import.meta.env.BASE_URL || "/").replace(/\/$/, ""))}/api/auth${path}`, {
    method: "POST",
    credentials: "include",
...
export const authApi = {
  getSession: async (): Promise<AuthSession | null> => {
    const res = await fetch(`${((import.meta.env.BASE_URL || "/").replace(/\/$/, ""))}/api/auth/get-session`, {
      credentials: "include",
  ...
  getProfile: async (): Promise<CurrentUserProfile> => {
    const res = await fetch(`${((import.meta.env.BASE_URL || "/").replace(/\/$/, ""))}/api/auth/profile`, {
      credentials: "include",
      headers: { Accept: "application/json" },
    });
    const payload = await res.json().catch(() => null);
