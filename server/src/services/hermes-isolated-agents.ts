...
function buildToolsGuide(role: string | null | undefined, name: string | null | undefined): string {
  ...
  return [
    ...
    "- **Email** → To read/check the inbox, call `emailListMessages` ONCE: it already",
    "  returns every sender + subject + date + status (up to 100) — that single list IS",
    "  the answer for \"read/check the emails\". Call `emailReadMessage` ONLY for a",
    "  specific message whose full body you genuinely need (e.g. to reply to it). NEVER",
    "  loop `emailReadMessage` over the whole inbox — reading every message one-by-one",
    "  times the run out before you can answer. To send, use `emailSendMessage` (only",
    "  when the task clearly asks to send to a real recipient).",
