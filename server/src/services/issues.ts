...
////      issueListSelect base64-encodes a preview of every `description`,
////      fans out to 5 parallel JOINs and the Drizzle/postgres.js path can
////      take seconds. Six UI components call issuesApi.list(companyId)
////      with no explicit limit — every render/refetch fires a 500-row
////      fetch. Stack a few open tabs and the pool collapses.
////      Cap to 50 (matches what UI viewports render anyway, virtualised
////      lists handle >50). Pagination unchanged (offset+explicit limit
////      still work, capped by ISSUE_LIST_MAX_LIMIT=1000).
//// Date: 2026-05-04
//// Refs: NORA #27 follow-up [[NORA/27-paperclip-neoffice-embed/README]]
...
export function issueService(db: Db) {
  ...
  return {
    ...
    list: async (companyId: string, filters?: IssueFilters) => {
      ...
      ////      MAX(activity_log)) is a correlated subquery per row against
      ////      issue_comments (1.09M rows on Osiris). EXPLAIN shows ~207ms
      ////      with LIMIT 50 (LIMIT applied AFTER ORDER BY materialises
      ////      every row). Postgres backends contend on LWLock BufferMapping
      ////      under concurrent polling. In Neoffice mode, fall back to
      ////      ORDER BY priority, updated_at, id — the bridge bumps
      ////      updated_at on every comment/heartbeat so the recency
      ////      semantics stay meaningful. Standalone Paperclip keeps the
      ////      canonical-last-activity ORDER BY.
      //// Date: 2026-05-04
      //// Refs: NORA #27 Phase M [[NORA/27-paperclip-neoffice-embed/README]]
      const NEOFFICE_DEPLOYMENT_ORDERBY = process.env.PAPERCLIP_DEPLOYMENT === "neoffice";
      const canonicalLastActivityAt = issueCanonicalLastActivityAtExpr(companyId);
      const orderByActivity = NEOFFICE_DEPLOYMENT_ORDERBY
      ...
      ////      subqueries against issue_comments AND activity_log per issue.
      ////      With 1.09M comments + 1.09M activity_log rows on Osiris these
      ////      scans hit LWLock BufferIO contention under concurrent polling.
      ////      Inbox.tsx fans out 5+ parallel fetches on every visit.
      ////      In Neoffice mode, fall back to updatedAt for every row. The
      ////      bridge bumps issues.updated_at on every comment/heartbeat so
      ////      the Inbox sort/grouping by activity recency stays meaningful.
      //// Date: 2026-05-04
      ...
      //// Why: listIssueBlockerAttentionMap, listIssueProductivityReviewMap,
      ////      and listIssueBlockedInboxAttentionMap all scan activity_log
      ////      (1M+ rows on Osiris) without a LIMIT to compute Inbox-only
      ////      badges. Neoffice's NORA UI does not surface these badges.
      ////      Skipping cuts ~80% of post-list scan time and prevents
      ////      BufferIO contention on concurrent calls (incident 2026-05-04).
      ////      Standalone Paperclip keeps the helpers — its Inbox uses the data.
      //// Date: 2026-05-04
      //// Refs: NORA #27 Phase M [[NORA/27-paperclip-neoffice-embed/README]]
      ...
      const [
        ...
        productivityReviewByIssueId,
        blockedInboxAttentionByIssueId,
      ] = NEOFFICE_DEPLOYMENT_ATTN
        ? [
            new Map() as Awaited<ReturnType<typeof listIssueBlockerAttentionMap>>,
            new Map() as Awaited<ReturnType<typeof listIssueProductivityReviewMap>>,
            new Map<string, IssueBlockedInboxAttention>(),
          ]
        : await Promise.all([
