  ...
  router.post("/plugins/:pluginId/bridge/action", async (req, res) => {
    ...
    try {
      const result = await bridgeDeps.workerManager.call(
        ...
        {
          key: body.key,
          //// Neocompany Modification — keep _actor/_noraTraceId enrichment, wrapped in the
          //// upstream authorized-company-scope guard + actorContext.
          params: actionParamsWithAuthorizedCompanyScope(enrichedParams, companyId),
